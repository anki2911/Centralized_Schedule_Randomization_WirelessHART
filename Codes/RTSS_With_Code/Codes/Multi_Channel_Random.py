...
def main():
    ...
    for i in range(hyperperiod):
        Slots.append([])          
        F_Seq.append([])
    f3 = open("Schedule_Default_26.txt","r")
    i = 0
    for lines in f3:
        lines = lines.strip()
        lines = lines.split(" ")
    ...
    #p = raw_input()
    select = 0
    for no in tqdm(range(int(num_of_exp))):
    ...
    print "Writing"                
    f = open("Entropy_Random_26.txt","w")
    g = open("entropy_out_26.txt","w")
    Ent = 0  
    ...
    for i in tqdm(range(len(Slots))):
        for j in range(num_of_chan):
            for k in range(len(FLOW)+1):
                if Prob[i][j][k] > 0:
                    Prob[i][j][k] = float(Prob[i][j][k])/(len(Slots)*num_of_chan*num_of_exp)
                    f.write(str(Prob[i][j][k]))
                    Ent = Ent - Prob[i][j][k] * math.log(Prob[i][j][k],2)
                else:
                    f.write(str(0))
                f.write("\t")
            f.write("\n")
